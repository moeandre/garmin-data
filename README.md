...
completa e filtrável de todas as corridas.
...
Duas fontes de dados alimentam o mesmo `report.json`, que serve tanto de
relatório quanto de **cache local**:
...
| [`scripts/fetch_garmin.py`](scripts/fetch_garmin.py) | API do Garmin Connect (login) | Atualizações de rotina — busca só o que é novo |
| [`scripts/build_page.py`](scripts/build_page.py) | `report.json` | Gera a página HTML a partir do relatório |
| [`scripts/garmin_common.py`](scripts/garmin_common.py) | — | Lógica compartilhada dos marcos de distância (usada pelos dois primeiros) |
...
Pede email/senha na primeira vez (veja [Autenticação](#autenticação-do-fetch_garminpy)
...
## Autenticação do `fetch_garmin.py`
